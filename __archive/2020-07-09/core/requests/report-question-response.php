...
if (
    ...
    if (
        ...
        $employeeQuery = "
            SELECT 
                PK_employee, firstName, middleName, lastName, birthDate, gender, mobileNo, AddressLine1, AddressLine2, AddressLine3, employeeNo, email, employee_type,
                IFNULL((
                    SELECT description 
                    FROM mscdepartment AS x 
                    WHERE x.PK_mscDepartment = FK_mscDepartment
                ), '-') AS `department`,
                IFNULL((
                    SELECT description 
                    FROM mscdivision AS x 
                    WHERE x.PK_mscDivision = FK_mscDivision
                ), '-') AS `division`
            FROM employees
            WHERE 
        ...
        if (count($employeeRecords) > 0) {
            foreach ($employeeRecords as $employeeRecord) {
                ...
                $csvContent[$arrayIndex][count($csvContent[$arrayIndex])] = $employeeRecord['department'];
                $csvContent[$arrayIndex][count($csvContent[$arrayIndex])] = $employeeRecord['division'];
