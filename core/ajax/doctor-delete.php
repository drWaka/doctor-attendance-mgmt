<?php
  ...
  if (isset($_POST['recordId'])) {
    $recordId = new form_validation($_POST['recordId'], 'int', 'Doctor Record ID', true);
    ...
    if ($recordId -> valid == 1) {
      if (is_numeric($recordId -> value)) {
        ...
        if (is_null($employee)) {
          $recordId -> valid = 0;
          $recordId -> err_msg = 'Doctor Record Not Found';
        }
      }
    }
    ...
    if ($recordId -> valid == 1) {
      ...
      $isDeleteSuccess = Employee::setInactive($recordId -> value);
      if ($isDeleteSuccess) {
        ...
        $response['content']['modal'] = modalize( 
          '<div class="row text-center">
              ...
              <p class="para-text col-12">Error Details: Unable to ' . $transactType['future'] . ' Doctor Record</p>
          </div>', 
          array(
            "trasnType" => 'error',
            "btnLbl" => 'Dismiss'
