<?php 
  ...
  function modalize($bodyContent, $footerButton, $modalSize = '') {
    ...
    if ($footerButton['trasnType'] == 'error') {
      ...
      $footerContent = '
        <div class="col-sm-12 text-center btn-container">
          <button type="button" class="btn btn-default update-rec-content modal-close transaction-btn transaction-success-button" data-target="' . $footerButton['container'] . '" data-link="' . $footerButton['link'] . '" trans-name="' . $footerButton['transName'] . '" data-content="' . $footerButton['content'] . '" data-dismiss="modal">' . $footerButton['btnLbl'] . '</button>
        </div>
      ';
